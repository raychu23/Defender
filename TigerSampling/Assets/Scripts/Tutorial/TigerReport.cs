...
public class TigerReport : MonoBehaviour
{
    ...
    public IEnumerator Upload(List<TigerData> tigerData)
    {
        ...
        //Fetch game number
        using (var www = UnityWebRequest.Post("https://stat2games.sites.grinnell.edu/php/gettigersamplingnum.php", numForm))
        {
        ...
        foreach (TigerData entry in tigerData)
        {
            ...
            form.AddField("Region", data["Region"]);
            ...
            using (var www = UnityWebRequest.Post("https://stat2games.sites.grinnell.edu/php/sendtigersamplinggameinfo.php", form))
            {
